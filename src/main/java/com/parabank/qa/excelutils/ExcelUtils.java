package com.parabank.qa.excelutils;
...
public class ExcelUtils {
	...
	/*
	 * public static void setCellDataWithMultipleValue(String xlfile, String
	 * xlsheet, int rownum, int colnum,String data) throws IOException { fi = new
	 * FileInputStream(xlfile); wb = new XSSFWorkbook(fi); ws =
	 * wb.getSheet(xlsheet); row = ws.getRow(rownum); cell = row.createCell(colnum);
	 * cell.setCellValue(data); List<Class> list = new ArrayList<Class>();
	 * Iterator<Row> itr = ws.iterator(); String newName = null; String oldName =
	 * null; while(itr.hasNext()){ Row nextRow = itr.next(); // For each row,
	 * iterate through all the columns Iterator<Cell> cellIterator =
	 * nextRow.cellIterator(); while (cellIterator.hasNext()) { Cell cell =
	 * cellIterator.next(); newName = nextRow.getCell(0).toString();
	 * if(nextRow.getCell(1).toString().contains(",")){ StringTokenizer st = new
	 * StringTokenizer(nextRow.getCell(1).toString(),",");
	 * while(st.hasMoreTokens()){ oldName = st.nextToken(); } } else{ oldName =
	 * nextRow.getCell(1).toString(); } } System.out.println(); } fo = new
